...
class Cuevana
  ...
  def extract_last_page
    ...
    return doc.at_xpath("/html/body/div/div[9]/span/a[6]").content.to_i
  end
  
  def extract_title(movie)
    return movie.at_css('.tit a').content
  end
  
  def extract_id(movie)
    return movie.at_css('.tit a @href').content.split('/')[2]
  end
  ...
  def extract_sources(movie)
    ...
    return player.css("html body div div div#sources ul script")
  end
  ...
  def extract_movies(pageNo)
    ...
    doc = Nokogiri::HTML(open(movies_url).read.force_encoding("utf-8"))
    return doc.xpath("//table//tr[@class != 'tabletit']")
  end
  ...
  def extract_download_link(key, host)
    ...
    return res.body.gsub(/[^a-zA-Z0-9\/\.\&\?=:]/, "") 
  end
